...
Both services are accessible by a single tenant/URI with the new release from SAP. Cloud Identity Services is bundled with the BTP (Business Technology Platform) contract, and you will have the option to activate two tenants, Test and Production, **free of charge** to connect to your SAP services. The Test tenant can be used for your Sandbox, Dev, and Testing systems, whereas your Production tenant can be used for the Production landscape. 

### Identity Authentication Services (IAS)  

The use of SAP Identity Authentication is commercially included in most SAP Business Technology Platform (SAP BTP) offerings under the General Terms and Conditions and can be utilized for all applications running on SAP BTP. 
...
**SAP Cloud Identity**, IAS, and IPS provide one test and one productive tenant (model) per customer, regardless of the number of contracts signed that include Identity Authentication.  
