...
**Caution**: If you already have SAP Cloud Identity Services activated, the steps below help you assign the service to an SAP BTP Subaccount.

### Getting started:

- To activate your Cloud Identity Services on SAP BTP, we need a subaccount to be created. SAP Cloud Identity Services runs on several underlying Infrastructure-as-a-Service (IaaS) technologies and regions, and can be activated on our cloud partner service providers, such as AWS (Amazon Web Services), Microsoft Azure, or GCP (Google Cloud Platform). You may refer to [Regional Availability](https://help.sap.com/docs/identity-authentication/identity-authentication/regional-availability?q=data%20center) before you choose/create your subaccount.   
...
Click **Configure Entitlements**, then select **Add Service Plans**. A new window should pop up. Select the service SAP Cloud Identity Services, choose the service **Default (Application)**, click on **Add 1 Service Plan**, and then **Save** the service plan.  
...
- The next screen gives you the option to create either a **TEST** tenant or a **PRODUCTIVE** tenant. We will select the **TEST** option for our first setup and click **Next**.
...
- You should be able to see the new pop-up window – creation in progress. Click on **View Subscription**, the page navigates to **Instances and Subscription**, and you should be able to see the service **Cloud Identity Service** with the status **Subscribed**.
