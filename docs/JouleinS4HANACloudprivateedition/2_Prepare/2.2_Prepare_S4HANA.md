...
Create a dedicated technical user in your SAP S/4HANA Cloud, private edition system with the role `SAP_FLP_EXP_USER`.

**What this user is used for**
- Used by the content exposure framework to **read the selected SAP Fiori launchpad content**  from the S/4HANA backend and make it available for **consumption in SAP BTP**.
- You typically use this user when configuring and running the **content exposure** steps.

## 2. Create Technical User for Identity Provisioning

Create a dedicated technical user in your SAP S/4HANA Cloud, private edition system with specific authorizations mentioned in [KBA 3666721](https://me.sap.com/notes/3666721) for Identity Provisioning.

**What this user is used for**
- Used as the **backend communication user** to support **identity provisioning** steps required for Joule setup.
- Enables the required **technical access** so users and authorizations can be provisioned/kept aligned as part of the overall integration.
...
## 4. Set Principal Propagation in Cloud Connector
