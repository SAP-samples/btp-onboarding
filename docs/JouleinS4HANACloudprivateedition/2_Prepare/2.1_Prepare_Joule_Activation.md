...
If the entitlements are not visible, it could be due to one of these reasons:
  ...
  * The start date for the Joule contract is at a future date hence the entitlements won't be visible in BTP until that date.
  ...
  In the scenarios above, please work with your Account Executive, BTP Customer Success Partner to resolve the entitlements issue prior to proceeding further with this mission.  It is also possible to acquire the Joule base entitlement from [SAP Store](https://www.sap.com/store.html).  Make sure to select the correct BTP Global Account when acquiring the Joule base entitlement through SAP Store.
...
2) What are the currently supported datacenters for Cloud Identity Service (Plan - Connectivity)?</br>
3) What is Cloud Identity Service tenant region?</br>
4) Which SAP solutions will Joule be set up for and what are datacenters of those solutions?</br>
5) Are there any legal requirements to choose a datacenter in a particular region?</br>
6) Do you have a preference for particular hyperscaler such AWS, Azure, Google etc.?</br>
