...
>
> - [Set Up Data Repository Integration | SAP Help Portal](https://help.sap.com/docs/joule/integrating-joule-with-sap/prepare-sharepoint-integration) — the official SAP guidance for this setup. It is written for SAP Joule for Business, but the data repository setup steps apply here as well.
> - [Understanding Resource Specific Consent for Microsoft Graph and SharePoint Online | Microsoft Learn](https://learn.microsoft.com/en-us/sharepoint/dev/sp-add-ins-modernize/understanding-rsc-for-msgraph-and-sharepoint-online) — Microsoft's reference on the `Sites.Selected` permission model that the steps below rely on.
